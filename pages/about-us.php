<?php include('../includes/header.php'); ?>
<link rel="stylesheet" href="../assets/css/about_us.css">

<div class="header-container">
        <div class="section__container" id="home">
            <div class="container-text">
                <h1>ABOUT</h1>
                <div class="page-path">
                    <p><span>Home ></span> ABOUT</p>
                </div>
            </div>
        </div>
    </div>

    <div class="sec">
        <div class="about-hero mb-5">
            <h2 class="text-center mb-4">About SBR Hotel Management</h2>
            <p class="text-center mb-5">We are dedicated to providing a comfortable and efficient stay for every guest.</p>
        </div>

<div class="container my-5 text-center">
  <!-- Logo -->
  <div class="mb-4">
    <img src="../assets/media/about/logo.jpg" alt="Secret Berry Resort Logo" class="img-fluid" style="max-height: 400px; max-width: 400px;">
  </div>

  <!-- Introduction -->
  <div class="mb-5 p-4 bg-light rounded shadow-sm" style="text-align: center;">
    <p>
      <strong>Secret Berry Resort</strong> is a luxury boutique hotel in Nuwara Eliya, catering to local and international tourists. We offer premium hospitality services including fine dining, guided tours, and wellness experiences — ensuring an unforgettable stay.
    </p>
    <p>
      Secret Berry Resort is a peaceful hideaway in the cool hills of Nuwara Eliya, Sri Lanka. Designed for those who seek comfort, nature, and luxury, our resort blends modern elegance with breathtaking natural surroundings.
    </p>
  </div>

  <!-- Confidentiality Statement -->
  <div class="mb-5 p-4 bg-white rounded shadow-sm border" style="text-align: center;">
    <h4 class="mb-3 text-danger"><i class="bi bi-shield-lock"></i> Confidentiality Statement</h4>
    <p>
      We offer a special welcome drink to all guests, crafted from a unique recipe that should not be disclosed to any third party. Only visitors who experience our resort can truly enjoy this one-of-a-kind flavor.
    </p>
  </div>

  <!-- Our Rooms -->
  <div class="mb-5 p-4 bg-light rounded shadow-sm" style="text-align: center;">
    <h4 class="mb-3 text-warning"><i class="bi bi-house-heart"></i> Our Rooms</h4>
    <p>
      From cozy Deluxe Rooms to the exquisite Luxury Suite, every room is designed to provide comfort, privacy, and a direct connection to nature. All rooms include high-speed Wi-Fi, room service, premium toiletries, and beautiful views.
    </p>
  </div>

  <!-- Location -->
  <div class="mb-5 p-4 bg-white rounded shadow-sm border" style="text-align: center;">
    <h4 class="mb-3 text-info"><i class="bi bi-geo-alt-fill"></i> Location</h4>
    <p>
      We are just minutes away from Gregory Lake, Victoria Park, and the city's vibrant market — yet hidden enough to offer complete peace and quiet.
    </p>
  </div>
</div>

<!-- Vision and Mission -->
<div class="container text-center">
  <div class="row vision-mission-container mb-5 justify-content-center">
    <div class="col-md-6 mb-4">
      <div class="vision-box p-4 bg-light rounded shadow-sm" style="text-align: justify;">
        <h3><i class="fas fa-eye me-2"></i> Our Vision</h3>
        <hr>
        <p>
          To revolutionize the hospitality industry by creating seamless hotel management solutions that empower hoteliers to deliver exceptional guest experiences while maximizing operational efficiency and profitability.
        </p>
      </div>
    </div>
    <div class="col-md-6 mb-4">
      <div class="mission-box p-4 bg-white rounded shadow-sm border" style="text-align: justify;">
        <h3><i class="fas fa-bullseye me-2"></i> Our Mission</h3>
        <hr>
        <p>
          We are committed to providing innovative, user-friendly hotel management systems that streamline operations, enhance guest satisfaction, and drive revenue growth through cutting-edge technology and personalized support.
        </p>
      </div>
    </div>
  </div>
</div>

        <!-- Team Section with Modified Layout -->
        <div class="team-section">
            <h2 class="text-center mb-5">Meet Our Leadership Team</h2>
            
            <div class="row">
                <!-- First Row: 3 Team Members -->
                <!-- Team Member 1 -->
                <div class="col-lg-4 col-md-4 mb-4">
                    <div class="team-member">
                        <div class="team-img mb-3">
                            <img src="../assets/media/about/chalana.jpeg" alt="CEO" class="img-fluid rounded">
                        </div>
                        <div class="team-info p-3">
                            <h4>Nimhara Chalana</h4>
                            <p class="position">Chief Executive Officer</p>
                            <p class="mb-2">With over 15 years of experience in hospitality management, Nimhara leads our company with vision and expertise.</p>
                            <div class="contact-info">
                                <p><i class="fas fa-envelope me-2"></i> nimhara@sbrhotel.com</p>
                                <p><i class="fas fa-phone me-2"></i> +1 (555) 123-4567</p>
                            </div>
                            <div class="social-links mt-3">
                                <a href="#" class="me-2"><i class="fab fa-linkedin"></i></a>
                                <a href="#" class="me-2"><i class="fab fa-twitter"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                
                <!-- Team Member 2 -->
                <div class="col-lg-4 col-md-4 mb-4">
                    <div class="team-member">
                        <div class="team-img mb-3">
                            <img src="../assets/media/about/dilusha.jpeg" alt="CTO" class="img-fluid rounded">
                        </div>
                        <div class="team-info p-3">
                            <h4>Dilusha Madushan</h4>
                            <p class="position">Chief Technology Officer</p>
                            <p class="mb-2">Dilusha oversees all technical aspects of our platform, ensuring cutting-edge solutions for our clients.</p>
                            <div class="contact-info">
                                <p><i class="fas fa-envelope me-2"></i> dilusha@sbrhotel.com</p>
                                <p><i class="fas fa-phone me-2"></i> +1 (555) 234-5678</p>
                            </div>
                            <div class="social-links mt-3">
                                <a href="#" class="me-2"><i class="fab fa-linkedin"></i></a>
                                <a href="#" class="me-2"><i class="fab fa-github"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                
                <!-- Team Member 3 -->
                <div class="col-lg-4 col-md-4 mb-4">
                    <div class="team-member">
                        <div class="team-img mb-3">
                            <img src="../assets/media/about/kaweesha.jpeg" alt="COO" class="img-fluid rounded">
                        </div>
                        <div class="team-info p-3">
                            <h4>Piyumi Kaweesha</h4>
                            <p class="position">Chief Operations Officer</p>
                            <p class="mb-2">Kaweesha ensures smooth operations and excellent service delivery for all our hotel partners worldwide.</p>
                            <div class="contact-info">
                                <p><i class="fas fa-envelope me-2"></i> piyumi@sbrhotel.com</p>
                                <p><i class="fas fa-phone me-2"></i> +1 (555) 345-6789</p>
                            </div>
                            <div class="social-links mt-3">
                                <a href="#" class="me-2"><i class="fab fa-linkedin"></i></a>
                                <a href="#" class="me-2"><i class="fab fa-instagram"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="row justify-content-center">
                <!-- Second Row: 2 Team Members -->
                <!-- Team Member 4 -->
                <div class="col-lg-4 col-md-5 mb-4">
                    <div class="team-member">
                        <div class="team-img mb-3">
                            <img src="../assets/media/about/maleesha.jpeg" alt="CMO" class="img-fluid rounded">
                        </div>
                        <div class="team-info p-3">
                            <h4>Maleesha Sadakalum</h4>
                            <p class="position">Chief Marketing Officer</p>
                            <p class="mb-2">Maleesha drives our brand strategy and marketing initiatives to connect hoteliers with our solutions.</p>
                            <div class="contact-info">
                                <p><i class="fas fa-envelope me-2"></i> maleesha@sbrhotel.com</p>
                                <p><i class="fas fa-phone me-2"></i> +1 (555) 456-7890</p>
                            </div>
                            <div class="social-links mt-3">
                                <a href="#" class="me-2"><i class="fab fa-linkedin"></i></a>
                                <a href="#" class="me-2"><i class="fab fa-twitter"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                
                <!-- Team Member 5 -->
                <div class="col-lg-4 col-md-5 mb-4">
                    <div class="team-member">
                        <div class="team-img mb-3">
                            <img src="../assets/media/about/ishan.jpeg" alt="CFO" class="img-fluid rounded">
                        </div>
                        <div class="team-info p-3">
                            <h4>Ishan Sivmal</h4>
                            <p class="position">Chief Financial Officer</p>
                            <p class="mb-2">Ishan manages our financial strategy with precision, ensuring sustainable growth and profitability.</p>
                            <div class="contact-info">
                                <p><i class="fas fa-envelope me-2"></i> ishan@sbrhotel.com</p>
                                <p><i class="fas fa-phone me-2"></i> +1 (555) 567-8901</p>
                            </div>
                            <div class="social-links mt-3">
                                <a href="#" class="me-2"><i class="fab fa-linkedin"></i></a>
                                <a href="#" class="me-2"><i class="fab fa-twitter"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

              <!-- Company Values Section -->
              <div class="values-section mt-5">
                <h2 class="text-center mb-4">Our Core Values</h2>
                <div class="row text-center">
                    <div class="col-md-3 mb-4">
                        <div class="value-item p-3">
                            <i class="fas fa-lightbulb fs-1 mb-3 text-primary"></i>
                            <h4>Innovation</h4>
                            <p>Constantly pushing boundaries to create better solutions</p>
                        </div>
                    </div>
                    <div class="col-md-3 mb-4">
                        <div class="value-item p-3">
                            <i class="lni lni-customer fs-1 mb-3 text-primary"></i>
                            <h4>Customer Focus</h4>
                            <p>Prioritizing our clients' needs in everything we do</p>
                        </div>
                    </div>
                    <div class="col-md-3 mb-4">
                        <div class="value-item p-3">
                            <i class="lni lni-handshake fs-1 mb-3 text-primary"></i>
                            <h4>Integrity</h4>
                            <p>Conducting business with honesty and transparency</p>
                        </div>
                    </div>
                    <div class="col-md-3 mb-4">
                        <div class="value-item p-3">
                            <i class="lni lni-grow fs-1 mb-3 text-primary"></i>
                            <h4>Excellence</h4>
                            <p>Striving for the highest quality in all our services</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>


    </div> 

<script src="../assets/js/about.js"></script>
<?php include('../includes/footer.php'); ?>
